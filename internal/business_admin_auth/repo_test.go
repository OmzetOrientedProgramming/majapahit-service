...
func TestRepo_CheckUserFields(t *testing.T) {
	...
	retrievedReturn := repoMock.CheckUserFields(*request)
	assert.Equal(t, retrievedReturn, expectedReturn)
}
...
func TestRepo_GetByPhoneNumber(t *testing.T) {
	phoneNumber := "089654273339"
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM users WHERE phone_number=$1 LIMIT 1")).
		WithArgs("089654273388").
		WillReturnRows(rows)

	retrievedReturn1, retrievedReturn2 := repoMock.GetByPhoneNumber(phoneNumber)
	assert.Equal(t, retrievedReturn1, nil)
	assert.Equal(t, retrievedReturn2, nil)
}
...
func TestRepo_CheckIfPlaceNameIsUnique(t *testing.T) {
	...
	t.Run("place name is not unique in database", func(t *testing.T) {
		...
		unique, err := repoMock.CheckIfEmailIsUnique("Kopi Kenangan Rawamangun")
		assert.NoError(t, err)
		assert.False(t, unique)
	})
	...
	t.Run("place name is unique in database", func(t *testing.T) {
		...
		unique, err := repoMock.CheckIfEmailIsUnique("Kopi Kenangan Pancoran")
		assert.NoError(t, err)
		assert.True(t, unique)
	})
	...
	t.Run("database error", func(t *testing.T) {
		...
		unique, err := repoMock.CheckIfBankAccountIsUnique("Kopi Kenangan Pasar Minggu")
		assert.Equal(t, ErrInternalServerError, errors.Cause(err))
		assert.False(t, unique)
	})
}
...
func TestRepo_CreateUser(t *testing.T) {
	...
	rows := mock.
		...
		AddRow()
	mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO users (phone_number, name, email, password, status) VALUES ($1, $2, $3, $4, $5) RETURNING phone_number, name, email")).
...
func TestRepo_CreateBusinessAdmin(t *testing.T) {
	userID := 1
	request := &RegisterBusinessAdminRequest{
		AdminBankAccount:     "008-123456789",
		AdminBankAccountName: "RAFI MUHAMMAD",
	}
	balance := 0.0
	...
	_ = mock.
		...
		AddRow()
	mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO business_owners (balance, bank_account, bank_account_name, user_id) VALUES ($1, $2, $3, $4)")).
		WithArgs(balance, request.AdminBankAccount, request.AdminBankAccountName, userID)
