...
func TestRepo_GetListCustomerBookingWwithPaginationSuccess(t *testing.T) {
	...
	SELECT b.id, u.name, b.capacity, b.date, b.start_time, b.end_time, b.status
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT COUNT(b.id) FROM bookings b, users u, places p WHERE p.user_id = $1 AND p.id = b.place_id AND u.id = b.user_id AND b.status = $2")).
...
func TestRepo_GetListCustomerBookingWithPaginationCountEmpty(t *testing.T) {
	listCustomerBookingExpected := &ListBooking{
		CustomerBookings: make([]CustomerBooking, 0),
		TotalCount:       0,
	}
...
func TestRepo_GetDetailSuccess(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT b.id, u.name, u.phone_number, u.image, b.place_id, b.date, b.start_time, b.end_time, b.capacity, b.status, b.total_price, b.created_at
...
func TestRepo_GetDetailInternalServerError(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT b.id, u.name, u.phone_number, u.image, b.date, b.start_time, b.end_time, b.capacity, b.status, b.total_price, b.created_at
		FROM bookings b, users u
		WHERE b.id = $1 AND b.user_id = u.id`)).
		WithArgs(bookingID).
		WillReturnError(sql.ErrTxDone)
